...
// dimension of used detector grid
const int DETECTOR_SCALES = 32;
const int DETECTOR_COLUMNS = 297;

// number history maps to filter out non continuous detections
const int TRAJECTOR_HISTORY = 12;
...
struct CCF_Info
{
	...
	double m_trajectMinVal;
	double m_trajectMaxVal;
	
	int m_trajectHistSize;	
};
...
protected:
	...
	// needed as tmp when redefining the grid
	float* m_gridTop_tmp;
	float* m_gridBottom_tmp;
	
	// number of scales and comlumns for the detector grid
	...
	// filter out 1-hit wonders and fill in missing detections
	Trajector* m_trajector;
	...
	//overlay with final confidence map
...
public:
	...
	CCF_Context(const int& numSlices=TRAJECTOR_HISTORY,const int& rows=DETECTOR_SCALES,const int& cols=DETECTOR_COLUMNS);
	virtual ~CCF_Context();
	...
	inline const cv::Mat& getDetectionMap(){return m_detectionMap;};
	inline const cv::Mat& getConfidenceMap(){return m_confidenceMap;};
	
	FreshDetector* getFreshDetector(){return m_freshDetect;};
	
	GaussianMixture* getMixtureModel(){return m_mixtureModel;};
	
	Trajector* getTrajector(){return m_trajector;};
