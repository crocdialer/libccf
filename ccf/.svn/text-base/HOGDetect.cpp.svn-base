...
size_t HOGDescriptorCCF::getDescriptorSize() const
{
...
double HOGDescriptorCCF::getWinSigma() const
{
    return winSigma >= 0 ? winSigma : (blockSize.width + blockSize.height)/8.;
}

bool HOGDescriptorCCF::checkDetectorSize() const
{
...
void HOGDescriptorCCF::setSVMDetector(const vector<float>& _svmDetector)
{
    svmDetector = _svmDetector;
    CV_Assert( checkDetectorSize() );
}

bool HOGDescriptorCCF::load(const String& filename, const String& objname)
{
...
void HOGDescriptorCCF::save(const String& filename, const String& objName) const
{
...
void HOGDescriptorCCF::computeGradient(const Mat& img, Mat& grad, Mat& qangle,
...
struct HOGCache
{
    ...
    HOGCache();
    HOGCache(const HOGDescriptorCCF* descriptor,
			 const Mat& img, Size paddingTL, Size paddingBR,
			 bool useCache, Size cacheStride);
    virtual ~HOGCache() {};
    virtual void init(const HOGDescriptorCCF* descriptor,
					  const Mat& img, Size paddingTL, Size paddingBR,
					  bool useCache, Size cacheStride);
    ...
    Mat grad, qangle;
    const HOGDescriptorCCF* descriptor;
};
...
HOGCache::HOGCache(const HOGDescriptorCCF* _descriptor,
...
void HOGCache::init(const HOGDescriptorCCF* _descriptor,
					const Mat& _img, Size _paddingTL, Size _paddingBR,
					bool _useCache, Size _cacheStride)
{
    ...
    //   1. loop over windows is unrolled in the HOGDescriptorCCF::{compute|detect} methods;
...
void HOGDescriptorCCF::compute(const Mat& img, vector<float>& descriptors,
...
void HOGDescriptorCCF::detect(const Mat& img,
							  vector<Point>& hits, vector<double>& score, double hitThreshold,
...
void HOGDescriptorCCF::detectMultiScale(
										const Mat& img, vector<Rect>& foundLocations, vector<double>& score, 
										...
										double scale0, int groupThreshold) const
{
	...
	scale=scale0; //changed to skip default scale
    ...
    for( levels = 0; levels < maxLevels; levels++ ) 
    {
		...
		levelScale[levels] = scale; //changed: after break
...
#pragma omp parallel for num_threads(nthreads) schedule(dynamic)
		...
		for( i = 0; i < levels; i++ )
		{
			...
			detect(smallerImg, tdata.locations, score, hitThreshold, winStride, padding);
...
vector<float> HOGDescriptorCCF::getDefaultPeopleDetector()
{
