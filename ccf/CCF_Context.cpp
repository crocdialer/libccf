...
//32 scales cam3 
//TODO : this geometric-information obviously needs to be generated by the program
// following geometric/camera calib info is for cam3 of skoviz dataset
...
double CCF_Context::ms_hogMIN = -6.650963;
double CCF_Context::ms_hogMAX = 1.693089;

using namespace std;
using namespace cv;

CCF_Context::CCF_Context(const int& numSlices,const int& rows,const int& cols):m_frameCounter(0),
m_detectorScales(rows),m_detectorColumns(cols),
m_dirtyContext(false),m_drawGrid(false),m_drawBoundingBoxes(true),
m_drawOverlayMap(false),m_boundingBoxThresh(.15),m_timer(NULL)
{
	...
	// Frangi Windows will have a overlap of ~20%
	m_frangiFilter = new FrangiFilter3D(numSlices,rows,cols,4*numSlices/5);
	m_infoStruct.m_frangiWindowSize = numSlices;
	
	m_detectionMap = Mat_<double>(rows,cols,0.0);
	m_confidenceMap = Mat_<double>(rows,cols,0.0);
	
	m_meanMap = Mat_<double>(rows,cols,0.0);
	...
	ms_colorMap = Colormap(Colormap::JET);
...
{	
	...
	catch (const exception& e ) 
	{
		try 
		{
			...
			Mat resized;
			resize (img, resized,Size(704,576));
			applyDetector(resized,m_detectionMap);
		}
		catch (...) 
		{
			// at this point the detector definitely failed
			// so we just draw the grid if requested and reset
			if(m_drawGrid) drawGrid(img);
			
			performReset();
			
			//printf("detector failed: %s\n",e.what());
	...
	m_mixtureModel->filter(m_detectionMap, m_confidenceMap);
	m_infoStruct.m_gaussianTime = m_timer->getElapsedTime();
	m_timer->reset();
	
	
	// Frangi filter will build up a stack and filter out and enhance trajectories
	m_frangiFilter->pushMap(m_confidenceMap);
	m_infoStruct.m_frangiCurrentIndex = m_frangiFilter->getCurrentIndex();
	...
	bundle.m_trajectorMap = m_confidenceMap;
	bundle.m_image = img.clone();
	bundle.m_frameInfo = m_infoStruct;
	
	// buffer bundle in queue
	m_ccfFrameQueue.push_back(bundle);
	
	if(!m_frangiFilter->isBuffering())
	{
		// get buffered frameBundle from queue 
		m_currentBundle = m_ccfFrameQueue.front();
		m_ccfFrameQueue.pop_front();
		
		// read back values
		m_detectionMap = m_currentBundle.m_detectionMap;
		m_confidenceMap = m_currentBundle.m_confidenceMap;
		img = m_currentBundle.m_image;
		
		// add current values for frangi
		m_frangiMap = m_currentBundle.m_trajectorMap = m_frangiFilter->getCurrentSlice().clone();
		
		bundle.m_detectRectangles = getDectectRectangles(m_frangiMap,img.size());
		
		m_meanMap += m_frangiMap;
	...
	for (int stripes=0; stripes<m_detectorScales; stripes++)
	{
		...
		switch (m_detectorType) 
		{
			case HOG_DETECTOR:
				...
				hogStride = Size(2,40); //Size(max(img.cols/352,1),max(img.rows/14,1))
				hogPadding = Size(24,16);//Size(max(img.cols/29,1),max(img.rows/36,1));
		...
		{		
			...
			for (int w=floor(avgWidth*i); w<upperLimit; w++) 
			{
				...
				detectMap.at<double>(stripes,w) = currentScore;
...
{	
	vector<Rect> rectangles = getDectectRectangles(m_frangiFilter->getCurrentSlice(),img.size());
...
void CCF_Context::performReset()
{
	...
	m_frangiFilter->reset();
	m_mixtureModel->resetBackground();
	m_ccfFrameQueue.clear();
