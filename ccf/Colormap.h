...
public:
	...
	cv::Mat apply(const cv::Mat& img1C);
	
	inline cv::Scalar valueFor(const unsigned char& v)
	{
		return cv::Scalar(m_byteValues[v]);
	}
	
	inline cv::Scalar valueFor(float f)
	{
		if(f < 0) f = 0; 
		else
			if(f > 1) f = 1;
	...
	inline cv::Scalar valueFor(const double& f){return valueFor((float)f);}
};
